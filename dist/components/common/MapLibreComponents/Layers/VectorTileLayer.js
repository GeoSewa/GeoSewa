...
export default function VectorTileLayer({ map, id, url, isMapLoaded, layerOptions, visibleOnMap = true, interactions = [], onFeatureSelect, }) {
    ...
    useEffect(() => {
        if (!map || !interactions.includes('select'))
            return () => { };
        function handleSelectInteraction(event) {
            if (!map)
                return;
            map.getCanvas().style.cursor = 'pointer';
            // @ts-ignore
